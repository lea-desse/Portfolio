...
const Contact: React.FC = () => {
  ...
  return (
    <footer id="contact" className="py-32 bg-slate-900 relative border-t border-slate-800 overflow-hidden text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 lg:pl-24 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
          <div>
            ...
            <div className="space-y-6">
              <a href={`mailto:${CV_DATA.email}`} className="flex items-center gap-6 p-6 rounded-3xl bg-slate-800/50 border border-slate-700 hover:border-green-500 transition-all group shadow-2xl backdrop-blur-xl">
                ...
                <div>
                  <p className="text-[9px] text-green-500 font-black uppercase tracking-widest mb-1">Transmission Channel</p>
                  <p className="font-black text-white text-md tracking-tight">{CV_DATA.email}</p>
                </div>
              </a>
              
              <div className="flex gap-4">
                <a href={CV_DATA.socials.linkedin} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center p-6 rounded-3xl bg-slate-800/50 border border-slate-700 hover:border-green-500 transition-all font-black text-[10px] uppercase tracking-widest text-white hover:text-green-400 shadow-2xl backdrop-blur-xl">
                  LinkedIn_Profile
                </a>
                <a 
                  href={`./${CV_DATA.cv_file}`} 
                  download
                  className="flex-1 flex items-center justify-center gap-3 p-6 rounded-3xl bg-green-500 hover:bg-green-400 border border-green-500 transition-all font-black text-[10px] uppercase tracking-widest text-slate-900 shadow-xl hover:scale-[1.02] active:scale-95"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                  {t('hero.cta_cv')}
                </a>
              </div>
            </div>
          ...
          <div className="bg-slate-800/50 p-10 md:p-14 rounded-[3rem] border-2 border-slate-700 shadow-2xl relative overflow-hidden group backdrop-blur-xl">
            {/* Background design */}
            ...
            {status === 'success' ? (
              ...
              <form className="space-y-8" onSubmit={handleSubmit}>
                ...
                <button 
                  ...
                  type="submit" 
                  className="w-full py-6 bg-green-500 hover:bg-green-400 disabled:bg-slate-800 disabled:text-slate-600 text-slate-900 rounded-2xl font-black uppercase tracking-[0.2em] text-[11px] transition-all shadow-xl hover:shadow-green-500/40 active:scale-95 flex items-center justify-center gap-3 group/btn"
                >
